...
def initial_fight(home_monster, away_monster):
    print("==========전투가 시작되었습니다!!==========")
    print(f"\n[{home_monster.name}]")
    print(f"타입/", home_monster.types)
    print(f"공격력/", home_monster.attack)
    print(f"방어력/", home_monster.defense)
    print(f"회피율/", home_monster.evasion)
    print("\nVS\n")
    print(f"\n[{away_monster.name}]")
    print(f"타입/", away_monster.types)
    print(f"공격력/", away_monster.attack)
    print(f"방어력/", away_monster.defense)
    print(f"회피율/", away_monster.evasion)
...
def fight(home_monster, away_monster, string_1_attack, string_2_attack):
    while (home_monster.bars>0) and (away_monster.bars>0):
        print(f"\n[유저][{home_monster.name}]\t{home_monster.bars}\t{home_monster.health}")
        print(f"[상대][{away_monster.name}]\t{away_monster.bars}\t{away_monster.health}\n")
...
def turn(home_monster, away_monster, effective, is_user):
    ...
    #회피율을 기준으로 공격이 빗나가면 체력을 소모하지 않음
...
if __name__=="__main__":
    ...
    #상대 몬스터 생성
    random_monster=["화끈몬", "축축몬", "수풀몬"]
    other_monster=create_monster(random.choice(random_monster))
